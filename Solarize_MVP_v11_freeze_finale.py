...
for index, row in fitted_temps.iterrows():
    date = datetime(year, 1, 1) + timedelta(index - 1)
...
# Plot the probability distribution plot
plt.plot(x, y,linewidth=2)
plt.xlabel('Temperature (Celcius)', fontsize=16)
plt.ylabel('Normalised Probability', fontsize=16)
plt.title(f'Probability Distribution Plot of Temperature on {day_of_month}/{month_name}/{year}', fontsize=16)
...
plt.yticks(month_positions, months)
...
from scipy.stats import norm
sd = 1
# Select a specific day and year
day = 170 # Change this to the day you want (1 to 365)
year = 2045 # Change this to the year you want (2024 to 2049)

# Get the mean and standard deviation of the watt-hour projection for that day and year
mean_change = annual_output_uncertainty_change.iloc[day-1, year-2024]
std_change = annual_output_uncertainty_change.iloc[day-1, year-2024]

mean_const = annual_output_uncertainty_const.iloc[day-1, year-2024]
std_const = annual_output_uncertainty_const.iloc[day-1, year-2024]

# Create a normal distribution object with the mean and standard deviation
dist_change = norm(loc=mean_change, scale=std_change)
dist_const = norm(loc=mean_const, scale=std_const)

# Create a range of values for the x-axis (watt-hour)
x_change = np.linspace(mean_change - sd*std_change, mean_change + sd*std_change, 100)
x_const = np.linspace(mean_const - sd*std_const, mean_const+ sd*std_const, 100)        

# Calculate the probability density function values for the x-axis
y_change = dist.pdf(x_change)
y_const = dist.pdf(x_const)
# Plot the probability distribution plot
plt.plot(x_change, y_change, label='Change', color='blue')
plt.plot(x_const, y_const, label='Const', color='red')
plt.xlabel('Watt-hour')
plt.ylabel('Probability')
plt.title(f'Probability Distribution Plot of Watt-hour on Day {day} of Year {year}')
plt.legend()
plt.show()
...
plt.fill_between(x, real_revenue - output_uncertainty_change, real_revenue + output_uncertainty_change, color='red', alpha=0.1)
