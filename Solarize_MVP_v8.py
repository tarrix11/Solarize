...
# I used Bayesian Ridge Regression on both solar irradiance and temperature 
# Fix temp uncertainty why big
...
# Create and fit the Bayesian Ridge Regression model on the training data
...
x_range = np.linspace(min(x1), max(x1), 365).reshape(-1, 1)
x_range_poly = poly.transform(x_range)
x_range_scaled = scaler.transform(x_range_poly)
y_pred, y_std = bayesian_model.predict(x_range_scaled, return_std=True)

plt.scatter(x1, y1, label='Averaged Irradiance', s=5)  # Set the size with the 's' parameter
...
plt.legend()
plt.title('Baysian Ridge Regression with Solar Irradiance')
...
# Ridge regression to model temperature to reduce noise and fill in missing data
import pandas as pd
import numpy as np
...
for year in all_temps_new.columns:
    ...
    plt.title(f'Ridge regression of temperatures for year: {year_number}')
    if year_number == '2018':
...
# Create an empty DataFrame to store the uncertainties
...
for index, row in fitted_temps.iterrows():
    ...
    y_pred = scaler_y.inverse_transform(y_pred_std.reshape(-1, 1))
    y_std = y_std_std * scaler_y.scale_
    ...
    # Unstandardize the future predictions and uncertainties
    y_future = scaler_y.inverse_transform(y_future_std.reshape(-1, 1))
    y_future_std = y_future_std_std * scaler_y.scale_
    ...
    # Add the uncertainties to the new DataFrame
    ...
    plt.title(f'Bayesian ridge regression on temperature data of day: {index}')
...
plt.figure(figsize=(15, 6))
# Plot the mean temperature and uncertainty
...
# Assuming efficiency_relative_uncertainity_change is your DataFrame
# and irradiance_relative_uncertainty is your numpy array
...
# Assuming df_efficiency and averaged_irradiance are your DataFrames
...
# I just changed the input to be 2020 instead of temps projection of year year 
...
# assuming x, changed_temps_rev, and constant_temps_rev are defined
...
plt.xlabel('Years elapsed since installation')  # replace with your x-axis label
plt.ylabel('Annual revenue (pounds)')  # replace with your y-axis label
plt.title('Revenue comparison per panel at Kew Gardens (1-sd error)')  # replace with your title
plt.legend()
