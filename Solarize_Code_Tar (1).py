...
# Reshape x to a 2D array (required for numpy.linalg.lstsq)
x_reshaped = x.reshape(-1, 1)

# Create the design matrix with polynomial features up to degree 6
X = np.column_stack([x_reshaped**i for i in range(4)])

# Use NumPy's least squares method
...
X_range = np.column_stack([x_range**i for i in range(4)])
y_pred = np.dot(X_range, coefficients)

plt.scatter(x, y, label='Data', s=5)
plt.plot(x_range, y_pred, color='red', label='Least Squares Error Fitted Curve')
plt.xlabel('Hours')
...
# Ridge Regression 
# Pick alpha then cross validate 
import numpy as np
...
plt.scatter(x, y, label='Data', s=5)  # Set the size with the 's' parameter
